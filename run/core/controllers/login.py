...
def signup():
    if request.method == 'GET':
        ...
        username = request.form['username']
        password = request.form['password']
        register(first,last,username,password,0,0,0,0)
        return redirect('/')

# For logging out: Remember session is a dict, so you have to
# clear it on the way out too, in order for new users to work!	
